...
class DataLawyerPathCorpus(object):
...
def lines_from_document(document: Path):
    """
    generator function to read in reviews from the file
    and un-escape the original line breaks in the text
    """
...
def tokenized_sentence_corpus(document):
    """
    generator function to use spaCy to parse reviews,
    lemmatize the text, and yield sentences
    """

    for doc in nlp.pipe(lines_from_document(document), batch_size=10000, n_threads=4):
        for sent in doc.sents:
            yield u' '.join([token.lower_ for token in sent if not punct_space(token)])
...
def tokenize_sentence_corpus(corpus_out_path, batch_id, documents):
    ...
    for document in documents:
        rows = []
        try:
            for sentence in tokenized_sentence_corpus(document):
                if len(sentence) > 0:
...
def process_texts(documents_path, year, court, corpus_out_path, batch_size=100, n_jobs=multiprocessing.cpu_count(),
    ...
    partitions = minibatch(DataLawyerPathCorpus(documents_path, year, court, debug), size=batch_size)
